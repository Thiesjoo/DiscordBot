...
const savePlayers = () => {
	...
	writableStream.on('finish', () => {
		encryptor.encryptFile(conf.fileNamePlayers, conf.encryptedPlayers, conf.encryptionKey, err => {
			// Encryption complete.
			if (err) {
	...
	game.playersBank.forEach(player => {
...
const saveHistory = () => {
	...
	writableStream.on('finish', () => {
		encryptor.encryptFile(conf.fileNameHistory, conf.encryptedHistory, conf.encryptionKey, err => {
			// Encryption complete.
			if (err) {
				return console.log(err);
			} else {
	...
	csvStream.pipe(writableStream);
	game.pickedNumbers.forEach(number => {
...
const gameResults = () => {
	...
	playersFromHand.forEach(player => {
...
const getNumber = () => {
	...
	if (tempNum === 0) {
		...
		text_channel.sendMessage('0 - Green', { tts: true });
	} else if (game.reds.indexOf(tempNum) > -1) {
		game.pickedNum = {
			type: 'red',
			number: tempNum
		};
		text_channel.sendMessage(`${tempNum} - Red`, { tts: true });
	} else if (game.blacks.indexOf(tempNum) > -1) {
		game.pickedNum = {
			type: 'black',
			number: tempNum
		};
		text_channel.sendMessage(`${tempNum} - Black`, { tts: true });
	}
...
client.on('ready', () => {
	...
	fs.access(conf.encryptedPlayers, err => {
		if (err && err.code === 'ENOENT') {
			createNewFiles();
		} else {
			// Decrypt file for Players & History
			encryptor.decryptFile(conf.encryptedPlayers, 'output_file.csv', conf.encryptionKey, err => {
				if (err) {
					createNewFiles();
				} else {
					// Decryption complete.
					csv.fromPath('output_file.csv').on('data', data => {
						var player = {
							name: data[0],
							id: data[1],
							bank: data[2],
	...
	fs.access(conf.encryptedHistory, err => {
		if (err && err.code === 'ENOENT') {
			saveHistory();
		} else {
			encryptor.decryptFile(conf.encryptedHistory, 'output_file_history.csv', conf.encryptionKey, err => {
				// Decryption complete.
				if (err) {
					console.log(err);
					saveHistory();
				} else {
					csv.fromPath('output_file_history.csv').on('data', data => {
						var historyNum = {
							type: data[0],
							number: data[1]
						};
...
client.on('message', message => {
	if (message.content.substring(0, 5).toLowerCase() === `${conf.prefix}play` && game.gameOn === false) {
		...
		if (game.betsClose === false) {
			var tempPlayer = game.playersBank.filter(player => player.id == message.author.id);
...
const grabNumbers = type => {
	return game.pickedNumbers.filter(numbers => numbers.type == type);
}

// Gets Player Index at a specificed player ID
const getIndex = value => {
	return game.playersBank.map(player => player.id).indexOf(value);
}

const getPlayer = value => {
	return game.playersBank.filter(player => player.id == value);
}
