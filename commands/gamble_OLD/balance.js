...
module.exports = [{
    ...
    async execute(msg, args) {
        ...
        let userBalance = await redis.hget(`disc:user:${mention}`, "balance")
        console.log(userBalance, await redis.hgetall(`disc:user:${mention}`))

        if (!userBalance) {
            let user = await db.initUser(mention)

            await redis.hset(`disc:user:${mention}`, `_id`, `${user._id}`, `balance`, `${user.balance}`, `symbol`, `${user.symbol}`, "EX", 60 * 60)
            //Cache balance for an hour

            userBalance = user.balance
        }

        msg.channel.send(`<@${mention}>'s balance is: $${userBalance}`)
    },
}, {
    name: 'bailout',
